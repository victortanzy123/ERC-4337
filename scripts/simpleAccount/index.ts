...
program
  ...
  .description("Swap WETH for Token in one action i.e. Flashswap on Uniswap")
  .option(
    "-dr, --dryRun",
    "Builds the UserOperation without calling eth_sendUserOperation"
  )
  ...
  .requiredOption(
    "-uniswapV2RouterAddress, --routerAddress <address>",
    "Uniswap V2 Router Address"
  )
  .requiredOption("-wethAddress, --weth <address>", "Weth Token Address")
  .requiredOption("-amt, --amount <decimal>", "Amount of the token to transfer")
  .action(async (opts) =>
    uniswapApproveAndSwapExactETHForTokens(
      opts.token,
      opts.routerAddress,
      opts.weth,
      opts.amount,
